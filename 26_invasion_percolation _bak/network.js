...
constructor(xMax,yMax){
    ...
    this.quedSequence = new Uint32Array(this.area).fill(-1);
    this.unquedSequence = new Uint32Array(this.area).fill(-1);
    this.invadedSequence = new Uint32Array(this.area).fill(-1);
    this.trappedSequence = new Uint32Array(this.area).fill(-1);
}
//Canvas Functions
resize(){
...
reset(){
    ...
    slider.max = this.canvas.width*this.canvas.height+1;
    //Start
    //this.startAnimation();
}
addQue(pixelIndex){
...
getInvasionSequence(){
    for(let i=0;i<this.canvas.width*this.canvas.height;i++){
        ...
        for(let throat=0;throat<4;throat++){
            if(xNeighbor[throat]>=this.canvas.width ||xNeighbor[throat]<0) break;
            if(yNeighbor[throat]>=this.canvas.height||yNeighbor[throat]<0) break;
...
updateField(){
    for(let index = 0; index<this.canvas.width*this.canvas.height; index++){
        if(this.invasionSequence[index]<this.sequence){
            //this.setPix(index,Math.floor(255*this.invasionSequence[index]/(this.canvas.width*this.canvas.height)));
            this.setPix(index, 0);
        }else{
