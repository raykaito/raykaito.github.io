...
class ImageData{
	...
	updateDisplayImage(){
		...
		for(let i=0;i<this.darea;i++){
			...
			xy[0]*=canvasScale;
			xy[1]*=canvasScale;
			this.dimgOut.data[4*i  ] = this.getPixelData(this.imgOut,xy, 0);
			this.dimgOut.data[4*i+1] = this.getPixelData(this.imgOut,xy, 1);
			this.dimgOut.data[4*i+2] = this.getPixelData(this.imgOut,xy, 2);
			this.dimgOut.data[4*i+3] = this.getPixelData(this.imgOut,xy, 3);
		}
	}
	...
	i2xy(i, width=this.width){
		return [i%width, Math.floor(i/this.dwidth)];
	}
	getPixelData(imgIn, xy, type){
		let indexai = this.xy2i([xy[0],xy[1]],imgIn.width);
		return imgIn.data[4*indexai+type];
	}
	getPixelDataI(imgIn, index, type){
...
class FindBlob extends ImageData{
	...
	updateDisplayImage(){
		...
		for(let i=0;i<this.darea;i++){
			if(this.getPixelDataI(this.dimgOut,i,1)==0&&this.dblobMap[i]!=(10*(this.maxAreaIndex+1))){
	...
	scanBlobs(){
		...
		for(let i=0;i<this.area;i++){
			if(this.getPixelDataI(this.imgOut,i,0)==255) continue;
			...
			while(tim!=-1){
				//go up until the end
				while(ty[tic]>=0&&this.getPixelData(this.imgOut, [tx[tic],ty[tic]-1], 0)==0){
					ty[tic]--;
				}
				...
				while(ty[tic]<this.height&&this.getPixelData(this.imgOut, [tx[tic],ty[tic]], 0)==0){
					...
					if(tx[tic]>0){
						if(left!=(this.getPixelData(this.imgOut, [tx[tic]-1,ty[tic]], 0)==0)){
							left = !left;
							if(left){
					...
					if(tx[tic]<this.width-1){
						if(right!=(this.getPixelData(this.imgOut, [tx[tic]+1,ty[tic]], 0)==0)){
							right = !right;
							if(right){
...
class Binarize extends ImageData{
	...
	updateHistogram(){
		...
		for(let i=0;i<this.imgIn.data.length;i+=4){
			index  = this.getPixelDataI(this.imgIn,i,0);
			index += this.getPixelDataI(this.imgIn,i,1);
			index += this.getPixelDataI(this.imgIn,i,2);
			index  = Math.floor(index/3);
			this.histogram[index]++;
		}
		this.max = this.histogram.reduce(function(a,b){return Math.max(a,b);},0);
	}
