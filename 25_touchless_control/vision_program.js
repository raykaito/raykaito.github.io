class VisionProgram{
    ...
    run(video){
        ...
        if(Math.abs(average(this.difference))>average(this.spreadFactor)/2){
            window.scrollBy(0,average(this.difference)*5);
        }
        if(average(this.spreadFactor)==NaN)alert("stop");
        console.log(this.counter+","+cmx+","+cmy+","+cwTotal+","+ccwTotal+","+average(this.difference)+","+average(this.spreadFactor)+","+variance);
        this.oimgdiff.data[4*(cmx+cmy*this.width)+0] = 255;
        this.oimgdiff.data[4*(cmx+cmy*this.width)+1] = 0;
        this.oimgdiff.data[4*(cmx+cmy*this.width)+2] = 0;
        ...
        this.dCanvas.fillRect(dWipeOffset+2*cmx-5,dWipeOffset+2*cmy-5,10,10,"red");
