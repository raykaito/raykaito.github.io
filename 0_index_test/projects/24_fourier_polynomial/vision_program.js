class VisionProgram{
    constructor(){
        ...
        this.dCanvas = new displayCanvas(document.getElementById('displayCanvas'),this);
        this.resizeOcanvas(1,1);

        this.histogram = new Histogram();
        this.blobFinder = new FindBlob();
    ...
    resizeOcanvas(width,height){
        ...
        this.width = width;
        this.height= height;
        this.dwidth = this.dCanvas.canvas.width;
        this.dheight= this.dCanvas.canvas.height;
    ...
    run(video){
        ...
        if(this.phase==0){
            ...
            const blobSelectionROI = this.newROI(this.width/4,(this.height)/4,width,height);
            this.histogram.autoBinarizeWithOtsuMethod(blobSelectionROI);

            //this.blobFinder.autoScanRemoveBorderAndExtractBlobs(this.histogram.passROI);
            //this.blobFinder.autoScanAndExtractLargest(this.blobFinder.passROI);
            //this.edgeExtraction.updateROI(this.blobFinder.passROI);
            //this.edgeExtraction.invert();
            //this.edgeExtraction.extractEdge(this.blobFinder.passROI);
            this.displayImageDataD(this.histogram);
            this.message = "Click Here to Pause Video";
        }else if(this.phase==1){
            if(this.xyClicked[1]<this.height*3/4){
                this.phase--;
                return;
            }
            this.blobFinder.autoScanRemoveBorderAndExtractBlobs(this.histogram.passROI);
            ...
            this.displayImageDataD(this.blobFinder);
            this.message = "Select Shapes and then click Here.";
        }else if(this.phase==3){
            if(this.xyClicked[1]>this.height*3/4){
                this.phase++;
            }else{
                this.blobFinder.selectBlob(this.xyClicked);
                this.phase--;
            }
    ...
    clicked([x,y]){
        this.phase++;
        this.xyClicked = [x/this.wScale,y/this.hScale];
    }
...
class Histogram extends ImageData{
    ...
    autoBinarizeWithOtsuMethod(ROI){
        ...
        this.getOtsuThresh();
        this.binarize();
        this.invertIfMinority();
    }
    invertIfMinority(){
        let pixelCount = 0;
        for(let i=this.thresh;i<this.bin.length;i++){
            pixelCount+=this.bin[i];
        }
        if(pixelCount<this.width*this.height/2){
            for(let i=0;i<this.width*this.height;i++){
                this.setPixI(i,255-this.getPixI(this.imgOut,i));
            }
        }
...
class Binary extends ImageData{
    constructor(){
        super();
    }
    updateROI(ROI){
