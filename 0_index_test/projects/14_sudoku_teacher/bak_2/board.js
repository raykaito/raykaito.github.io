...
userInput(x,y,num){
	...
	if(this.phase==2){
		if(this.notes[x][y][num-1]==-1)	this.notes[x][y][num-1]=this.step;
		else							this.notes[x][y][num-1]=-1;
	}
	draw();
}
showHideMethod(){
...
erasedNoteFound(x,y,num,met,par=0){
	...
	this.notes[x][y][num-1]=this.step;
	this.step++;
		...
		for(let x=0;x<9;x++){
			for(let y=0;y<9;y++){
				...
				for(let num=0;num<9;num++){
					if(this.notes[x][y][num]>=slider.value||this.notes[x][y][num]==-1)drawNotes(x+1,y+1,num+1,"darkGray");
				}
			}
		}
	}
...
solve(){
	...
	if(this.noCandidate()) return true;
	return false;
}
noCandidate(){
	...
	for(let box=0;box<9;box++){
		...
		let yCandidateLine = -1;
			for(let i=0;i<9;i++){
				...
				if(this.tiles[x][y]!=0) continue;
				if(this.notes[x][y][num-1]>-1) continue;
	...
	for(let x=0;x<9;x++){
		for(let y=0;y<9;y++){
			...
			if(num!=0){
				for(let i=0;i<9;i++){
					//By column
					if(this.notes[i][y][num-1]==-1)this.notes[i][y][num-1]=this.step;
					//By row
					if(this.notes[i][y][num-1]==-1)this.notes[x][i][num-1]=this.step;
					//By box
					const box = indexToBox([x,y]);//0~9
					const [xtemp,ytemp] = this.biToXY([box,i]);
					if(this.notes[i][y][num-1]==-1)this.notes[xtemp][ytemp][num-1]=this.step;
				}
			}
		}
	}
