class VisionProgram_SudokuReader{
	...
	findFourCorners([angle, xin, yin]){
		...
		if(Math.min(sexy[0],swxy[0],nexy[0],nwxy[0])==-1) return "fail";
		...
		for(let i=0;i<10;i++){
			...
			if(index!="fail"){
				...
				if(indexCounter==2){
					...
					if(startingPoint[0]==0&&startingPoint[1]==0) return "fail";
	...
	findAngle(xin,yin){
		const acceptableLineIntensityratio = 5;
		const tryStep = this.cellMax/10;
		const tryWidth= tryStep*2;
		let tryCounter = 1;
		let angleTemp = 90;
		let xyStep = getXYfromDirDis(angleTemp,tryStep);
		let xNow = xin+xyStep[0];
		let yNow = yin+xyStep[1];
		while(yNow<hcanvas.height){
			const img = newWindow().centerWidthHeight(xNow,yNow,tryWidth,1);
			const scanner = new IntersectionDetector(img.passdata,0);
			const array = scanner.lineIntensitySorted;
			if((array[0][0]/array[1][0])>acceptableLineIntensityratio){
				angleTemp = getDir([xin,yin],[scanner.xpos+array[0][1],scanner.ypos]);
				circle(scanner.xpos+array[0][1],scanner.ypos,3);
			}
				tryCounter++;
				xyStep = getXYfromDirDis(angleTemp,tryStep*tryCounter);
				xNow = xin+xyStep[0];
				yNow = yin+xyStep[1];
		}
		return;
	...
	findAngleReliable(xin, yin, dirIn){
		const tryCount = 10;
		const xyStep = getXYfromDirDis(dirIn,2*this.cellMax/tryCount);
		let successCounter = 0;
		...
		for(let i=0;i<tryCount;i++){
			...
			const scanner = new IntersectionDetector(img.passdata,0,1);
			const array = scanner.lineIntensity;
	...
	findAngleCandidate(xin, yin){
		...
		let scannerUpper = new IntersectionDetector(imgDataUpper.passdata,0,1);
		let scannerLower = new IntersectionDetector(imgDataLower.passdata,0,1);
