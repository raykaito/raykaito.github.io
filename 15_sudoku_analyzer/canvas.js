...
const drawLine = (xi,yi,xii,yii,w) => {
    ...
    yi = Math.floor(yi )+(widthOdd?0.0:0)+(vertical?(yiBigger? ct.lineWidth:-ct.lineWidth):0)/2;
    yii= Math.floor(yii)+(widthOdd?0.0:0)+(vertical?(yiBigger?-ct.lineWidth: ct.lineWidth):0)/2;
    ct.beginPath();
    ct.moveTo(xi ,yi );
    ct.lineTo(xii,yii);
    ct.stroke();
